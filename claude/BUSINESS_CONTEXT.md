...
```mermaid
journey
    title B2B Buyer Journey
    section Awareness
      Searches for logistics partner: 3: Buyer
      Lands on site via search/referral: 4: Buyer
    section Evaluation
      Scans hero for credibility signal: 4: Buyer
      Reviews services and trade lanes: 4: Buyer
      Checks trust signals / social proof: 5: Buyer
    section Decision
      Requests a quote: 5: Buyer
      Receives prompt, specific response: 5: Buyer
    section Retention
      Onboarded as client: 5: Buyer
      Refers or returns for new lane: 5: Buyer
```
...
- **Personal contact exposure — reopened, then widened by the client.** The original resolution published one general channel (saravanakumaar@skinternationals.in, +91 88700 15754) and withheld the brochure's named mobile numbers. The client has since designated two desks for public display — Customer Service & Coordination and Operations, with the named addresses and mobiles held in `content/contacts.ts` — which now appear in the Contact section and the footer. The general channel is retained beside them for enquirers who don't yet know which desk they need, and the quote form remains the primary conversion path.
